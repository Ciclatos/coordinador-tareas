...
describe("configuración independiente por sección", () => {
  it("inicia y limpia con rango completo, desde 1 y final vacío", () => {
    expect(emptySection("new")).toMatchObject({
      selection: "range",
      start: 1,
      end: null,
      interval: 1,
    ...
    expect(() => generateSectionLabels(emptySection("new"))).toThrow(
      "El rango inicial y final no es válido",
    );
  });
  it("genera 6, 10 y 4 ejercicios sin compartir configuración", () => {
    ...
    const c = {
      ...
      manualList: "5,10,15,20",
    };
    expect([
      generateSectionLabels(a).length,
      generateSectionLabels(b).length,
      generateSectionLabels(c).length,
    ]).toEqual([6, 10, 4]);
