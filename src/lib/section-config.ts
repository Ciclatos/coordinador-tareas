...
export const emptySection = (id: string, name = "Sección 1"): SectionConfig => ({
  id,
  name,
  selection: "multiple",
  start: 5,
...
export function duplicateLabels(value: string) {
  ...
  return [...new Set(parseLabelList(value).filter((item) => seen.has(item) || !seen.add(item)))];
}

export function generateSectionLabels(section: SectionConfig) {
  if (section.selection === "manual")
    return [...new Set(parseLabelList(section.manualList))].sort(naturalCompare);
  ...
  if (section.end - section.start > 1000) throw new Error("El rango no puede superar 1000 posiciones.");
  ...
  const generated = Array.from(
    ...
    (section.selection === "range" && (value - section.start) % section.interval === 0) ||
    (section.selection === "odd" && value % 2 === 1) ||
    (section.selection === "even" && value % 2 === 0) ||
    (section.selection === "multiple" && value % section.interval === 0),
  ).map(String).filter((label) => !excluded.has(label));
  return [...new Set([...generated, ...included])].sort(naturalCompare);
}

export function sectionRule(section: SectionConfig, mode: string, seed: string) {
  return {
    version: 2,
    mode,
    seed,
...
export function sectionFromStored(input: {
  ...
  const rule = input.rule && typeof input.rule === "object"
    ? input.rule as Record<string, unknown>
    : {};
  const selection = ["range", "odd", "even", "multiple", "manual"].includes(String(rule.selection))
    ? rule.selection as SectionSelection
    : input.labels.length ? "manual" : fallback.selection;
  return {
    ...fallback,
    selection,
    start: typeof rule.start === "number" ? rule.start : fallback.start,
    end: typeof rule.end === "number" ? rule.end : fallback.end,
    interval: typeof rule.interval === "number" ? rule.interval : fallback.interval,
    manualList: typeof rule.manualList === "string"
      ? rule.manualList
      : selection === "manual" ? input.labels.join(", ") : "",
